// lib/widgets/charts/streak_chart.dart
...
class StreakChart extends StatelessWidget {
  final Map<DateTime, int> dailyStreakData; // Key: Date, Value: Streak count on that date

  const StreakChart({super.key, required this.dailyStreakData});

  @override
  Widget build(BuildContext context) {
    if (dailyStreakData.isEmpty) {
      return Center(
        child: Text(
          'No streak data to display yet.',
    ...
    // Sort data by date
    final sortedDates = dailyStreakData.keys.toList()..sort();
    ...
    for (int i = 0; i < sortedDates.length; i++) {
      ...
      final streak = dailyStreakData[date] ?? 0;
      spots.add(FlSpot(i.toDouble(), streak.toDouble()));
      if (i > maxX) maxX = i.toDouble();
      if (streak > maxY) maxY = streak.toDouble();
    }

    // Add padding to maxY for better visualization
    maxY = (maxY * 1.2).ceilToDouble();
    if (maxY < 5) maxY = 5; // Ensure a minimum scale for visibility
              ...
              titlesData: FlTitlesData(
                ...
                leftTitles: AxisTitles(
                  sideTitles: SideTitles(
                    ...
                    interval: maxY / 4 > 1 ? (maxY / 4).ceilToDouble() : 1, // Adjust interval dynamically
                  ),
                ),
              ),
              ...
              lineBarsData: [
                LineChartBarData(
                  ...
                  gradient: LinearGradient(
                    colors: [
                      AppColors.streakColor.withOpacity(0.8),
                  ...
                  belowBarData: BarAreaData(
                    ...
                    gradient: LinearGradient(
                      colors: [
                        AppColors.streakColor.withOpacity(0.3),
