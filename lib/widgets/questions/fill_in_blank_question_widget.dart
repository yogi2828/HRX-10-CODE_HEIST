...
class _FillInBlankQuestionWidgetState extends State<FillInBlankQuestionWidget> {
  ...
  Future<void> _checkAnswer() async {
    ...
    final correctAnswer = (widget.question.correctAnswer as String).trim();
    ...
    if (widget.question.expectedAnswerKeywords != null && widget.question.expectedAnswerKeywords!.isNotEmpty) {
      try {
        ...
        final prompt = "Evaluate if the user's answer semantically matches the expected keywords. Provide a boolean 'isCorrect' and a 'feedback' string. User Answer: '$userAnswer'. Expected Keywords/Concepts: '${widget.question.expectedAnswerKeywords}'. Correct Answer: '$correctAnswer'.\n\nExample JSON format: {\"isCorrect\": true, \"feedback\": \"Great job!\"}";
        final schema = {
          "type": "OBJECT",
          "properties": {
        ...
        feedbackMessage = correct ? 'Correct!' : 'Not quite. The answer was "$correctAnswer".';
      }
    } else {
      correct = userAnswer.toLowerCase() == correctAnswer.toLowerCase();
      feedbackMessage = correct ? 'Correct!' : 'Not quite. The answer was "$correctAnswer".';
